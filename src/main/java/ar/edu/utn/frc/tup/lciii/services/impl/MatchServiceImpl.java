...
public class MatchServiceImpl implements MatchService {
    ...
    public List<MatchResponseDTO> getMatchesByPlayer(Long playerId) {
        ...
        if(matchEntityList.isPresent()) {
            ...
            matchEntityList.get().forEach(me -> {
    ...
    public RoundResponseDTO createRoundMatch(Long matchId, NewMatchRoundRequestDTO newMatchRoundRequestDTO) {
        ...
        // 2
        if(!getPlayingMatch(newMatchRoundRequestDTO.getPlayerId()).isPresent()){
            throw new EntityNotFoundException(String.format("The match id " + matchId + " does not belong to player " + newMatchRoundRequestDTO.getPlayerId()));
        }
        // 3
        ...
        if(player.getBalanceChips().intValue() < 20){
            throw new ResponseStatusException(HttpStatusCode.valueOf(403), String.format("Insufficient balance"));
        }
        // 5 ------INICIO--------
    ...
    public RoundResponseDTO playRoundMatch(Long matchId, Long roundId, RoundPlayDTO roundPlayDTO) {
        ...
        if (round == null){
            throw new EntityNotFoundException(String.format("The round id " + roundId + " not found"));
        }
        // 3
        if(!getMatchesByPlayer(roundPlayDTO.getPlayerId()).isEmpty()){
            throw new EntityNotFoundException(String.format("The match id " + matchId + " does not belong to player " + roundPlayDTO.getPlayerId()));
        }
        // 4
        ...
        // 5
        if(Objects.nonNull(round.getWinner()) || round.getRoundHandStatusPlayer() != RoundHandStatus.IN_GAME){
            throw new ResponseStatusException(HttpStatusCode.valueOf(403), String.format("Round is end"));
        }
        // 6
    ...
    public Optional<Match> getPlayingMatch(Long playerId) {
        ...
        if(matchEntityList.isPresent() && matchEntityList.get().size() != 0){
    ...
    private void playAppRound(Round round, Long playerId) {
        ...
        while(round.getRoundHandStatusApp().equals(RoundHandStatus.IN_GAME)) {
            ...
            BigDecimal value = getCardsValue(round.getAppCards());
            // 4 - Calcular el estado del round para la app -> Ayuda: calculateAppHand
            round.setRoundHandStatusApp(calculateAppHand(value));
        }
    ...
    private void calculateWinner(Round round, Long playerId) {
        if(round.getRoundHandStatusPlayer().equals(RoundHandStatus.EXCEEDED)) {
            if(round.getRoundHandStatusApp().equals(RoundHandStatus.EXCEEDED)) {
                ...
                playerService.updatePlayerBalance(playerId, BigDecimal.valueOf(round.getChipsInPlay().intValue() - CHIPS_PER_ROUND.intValue()));
            }
        } else {
            if(round.getRoundHandStatusApp().equals(RoundHandStatus.EXCEEDED)) {
                ...
                if(round.getPlayerCardsValue().compareTo(CARDS_LIMIT) == 0) {
                    ...
                    playerService.updatePlayerBalance(playerId, BigDecimal.valueOf(round.getChipsInPlay().intValue() * 2));
                } else {
    ...
    private RoundHandStatus calculateAppHand(BigDecimal cardsValue) {
        ...
        if (cardsValue.compareTo(new BigDecimal(7.5)) > 0) {
            return RoundHandStatus.EXCEEDED;
        } else if(cardsValue.compareTo(new BigDecimal(5)) > 0) {
